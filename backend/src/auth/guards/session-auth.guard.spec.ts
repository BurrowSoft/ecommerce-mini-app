...
function makeContext(session: Record<string, unknown> | undefined): ExecutionContext {
  return {
    switchToHttp: () => ({
      getRequest: () => ({ session }),
    }),
...
describe("SessionAuthGuard", () => {
  ...
  it("rejects when the session has no userId", () => {
    const guard = new SessionAuthGuard();
    expect(() => guard.canActivate(makeContext({ lastSeenAt: Date.now() }))).toThrow(
  ...
  it("rejects and destroys the session once idle time exceeds the configured timeout", () => {
    ...
    expect(() => guard.canActivate(makeContext(session))).toThrow(UnauthorizedException);
